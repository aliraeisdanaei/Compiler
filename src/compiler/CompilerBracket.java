...
/**
 * This class compiles the following grammar:
 * <p>
 * S → [S] | {S} | (S) | SS | e
 * e as in the empty string
 * </p>
 * <p>
 * to simplify the compilation the grammar has been converted to chomsky's normal form as below:
 * The empty string production has also been replaced by 14) - 16) for simplification
 * 0)S → S S
 * 1)B10 → (
 * 2)B11 → )
 * 3)B20 → {
...
public class CompilerBracket {
    ...
    /**
     * this assumes that the input has gone through a lexical analyzer & each char in the string is a token
     */
    private String tokens;

    /**
     * the arraylist that holds each production, each production is an array of Strings
     * the first element of each array is always the left hand side & the other two elements are the right hand side,
     * will have 2 or 3 elements depending on the production
     */
    private ArrayList <String []> grammarProductions = new ArrayList <> ();

    /**
     * will create a compiler with the input of token strings provided. & instantiates the grammar
     * @param tokens is the input of string the grammar needs to  be checked on
     */
    public CompilerBracket(String tokens){
        this.tokens = tokens;
     ...
     * works by running 5 operations of accept, reduce, shift, backtrack (omitted for this grammar), &
     * reject in order of successive fails
     * @return true iff the input can be derived with the grammar defined in the documentation of this class
     */
...
//        }
        //  while( (! this.tokens.isEmpty()) && this.tokenStack.size() != 1){
        //  System.out.println("The LOOOP Iteration-------------------------------------");
        if(accept()){
...
//                    System.out.println("Shift not possible anymore, the string was not in grammar");
     ...
     * @return the index of the production reduction has been applied & -1 when reduction was not possible
     */
    private int reduce(){
...
//                    System.out.println("used this production: " + i);
     ...
     * @return true iff shift was completed returns false when the {@code tokens} was empty & shift was not possible
     */
